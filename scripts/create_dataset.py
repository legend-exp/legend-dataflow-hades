...
for f in files:
    ...
    # key = smk.io.expand("{key_pattern()}", detector = d["detector"], measurement = d["measurement"], run = d["run"], timestamp = d["timestamp"])[0]
    key = "-".join(list(d.values())) # equivalent, but faster
    keys.append(key)
