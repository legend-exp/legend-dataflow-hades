...
wf_idxs = om.event_selection(raw_file, config_dict, db_dict, np.array([238.632,   583.191, 727.330, 860.564, 1620.5, 2614.553]), 5, (50,50))
...
out_grids = om.run_optimisation_multiprocessed(raw_file, opt_dict, config_dict, db_dict = db_dict, 
            ...
            processes=15, parameter=parameters, func=pgf.radford_peak,
            peak=2614.553, kev_width=(50,50))

ftp_dict = {}
for i in range(len(out_grids)):
    values = np.array([out_grids[i][j]['fwhm']/out_grids[i][j]['max'] for j in range(len(out_grids[i]))])
