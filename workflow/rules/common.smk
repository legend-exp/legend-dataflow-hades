...
from hadesflow.methods.utils import run_splitter, convert_to_daq_timestamp, convert_to_daq_run
...
def get_par_file(wildcards, tier):
    ...
    if Path(str(get_pattern_tier_daq(config).parent).replace("{detector}", wildcards.detector).replace("{measurement}", "th_HS2_lat_psa")).is_dir():
    ...
    return ProcessingFileKey.get_full_path_from_filename(files[0], 
...
def get_config_files(config_db, timestamp, measurement, channel, rule_name, field):
    ...
    rule_dict = config_db.valid_for(timestamp, system=measurement)["snakemake_rules"][rule_name][
            "inputs"
        ][field]
   
    return ro(
         rule_dict[channel] if channel in rule_dict else rule_dict["__default__"]
    )
